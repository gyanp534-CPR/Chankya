...
function formatOption(index: number, text: string): string {
  const letter = String.fromCharCode("A".charCodeAt(0) + index);
  return `${letter}) ${text}`;
}

function loadRows(filePath: string): NormalizedRow[] {
  const raw = fs.readFileSync(filePath, "utf-8");
  return JSON.parse(raw.replace(/^\uFEFF/, "")) as NormalizedRow[];
}
...
function exportYear(year: number) {
...
function writeSummary(results: Array<ReturnType<typeof exportYear>>) {
  const lines: string[] = [];
